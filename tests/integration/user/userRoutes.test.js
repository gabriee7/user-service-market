...
describe('User Routes', () => {
  ...
  it('deve retornar 400 para payload inválido em POST /user/api/users', async () => {
    // Arrange
    const payload = {};
    // Action
    const res = await request(app).post('/user/api/users').send(payload);
    // Assert
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty('error');
  });

  it('deve criar usuário com sucesso em POST /user/api/users', async () => {
    // Arrange
    ...
    // Action
    const res = await request(app).post('/user/api/users').send(dto);
    // Assert
  ...
  it('deve retornar lista de usuários em GET /user/api/users', async () => {
    // Arrange
    const rows = [{ id: 'u1', name: 'A', email: 'a@example.com' }];
    userService.getAll = jest.fn().mockResolvedValue(rows);
    // Action
    const res = await request(app).get('/user/api/users');
    // Assert
  ...
  it('deve retornar usuário por id em GET /user/api/users/:id', async () => {
    // Arrange
    const user = { id: 'u1', name: 'A', email: 'a@example.com' };
    userService.get = jest.fn().mockResolvedValue(user);
    // Action
    const res = await request(app).get('/user/api/users/u1');
    // Assert
  ...
  it('deve retornar 404 quando GET /user/api/users/:id não encontrar usuário', async () => {
    // Arrange
    userService.get = jest.fn().mockRejectedValue(new NotFoundException('User not found'));
    // Action
    const res = await request(app).get('/user/api/users/missing');
    // Assert
