...
describe('User Routes', () => {
  ...
  it('deve retornar 400 para payload inválido em POST /user', async () => {
    // Arrange
    const payload = {};
    // Action
  ...
  it('deve criar usuário com sucesso em POST /user', async () => {
    // Arrange
    ...
    // Action
    const res = await request(app).post('/user').send(dto);
    // Assert
  ...
  it('deve retornar lista de usuários em GET /user', async () => {
    // Arrange
  ...
  it('deve retornar usuário por id em GET /user/:id', async () => {
    // Arrange
  ...
  it('deve retornar 404 quando GET /user/:id não encontrar usuário', async () => {
    // Arrange
    userService.get = jest.fn().mockRejectedValue(new NotFoundException('User not found'));
    // Action
    const res = await request(app).get('/user/missing');
    // Assert
